...
eta = 0.02;
Du = deltaFunction(xs, ys, Xu, Yu, eta);
Dv = deltaFunction(xs, ys, Xv, Yv, eta);
figure,
surf(Xu, Yu, Du)
% waitforbuttonpress
% ----------------------------------------------------------------------- %
...
for k = 1:nt
    ...
    VX = zeros(length(ys), 1);
    for i=1:n
        Du = deltaFunction(xs(i), ys(i), Xu, Yu, eta);
        Dv = deltaFunction(xs(i), ys(i), Xv, Yv, eta);
        UX(i) = trapz(yu, trapz(xu, Du .* U));
        VX(i) = trapz(yv, trapz(xv, Dv .* V));
        fxt(i) = fxt(i) + (UX(i) - 0) * dt;
        fx0 = UX(i) - 0;
        fyt(i) = fyt(i) + (VX(i) - 0) * dt;
        fy0 = VX(i) - 0;
        fx = alpha * fxt(i) + beta * fx0;
        fy = alpha * fyt(i) + beta * fy0;
        Fx = Fx + fx * Du;
        Fy = Fy + fy * Dv;
    end
%     figure(1),
